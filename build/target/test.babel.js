...
(function (window) {
  ...
  window.currentLanguage = window.getLanguageCookies() || window.LANGUAGE_BY_ENV[window.SDP_ENV] || navigator.language || navigator.userLanguage;

  window.isRtl = targetSelector => {
    try {
      return window?.currentLanguage.indexOf('ar') !== -1 || window.getComputedStyle(document.querySelector('html')).direction === 'rtl' || window.getComputedStyle(document.querySelector('body')).direction === 'rtl' || !!targetSelector && window.getComputedStyle(document.querySelector(targetSelector))?.direction === 'rtl';
    } catch (error) {
      return false;
    }
  };
